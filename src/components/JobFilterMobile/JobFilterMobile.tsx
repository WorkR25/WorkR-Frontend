...
import { setExperience, setInternshipType, setMinSalary, setMinStipend, setOfficeType, setOpenFilter } from '@/lib/features/jobs/jobSlice';
...
const JobFilterMobile: FC<JobFilterProps> = ({ jobCategory, handleMoreFilters, clearParams }) => {
  ...
  return (
    <div className='w-full absolute h-screen z-[998] flex items-center justify-center'>
      <div className='flex w-full items-center justify-center absolute top-0 z-[999] md:p-20 py-20 p-1 h-full bg-black bg-opacity-60 '>
        <div className='flex flex-col gap-4 w-full'>
          <div className='py-5 px-8 flex flex-col items-start justify-center w-full bg-white border-[2px] border-background rounded-lg h-fit gap-8'>
            <div className='w-full flex justify-between items-center gap-4'>
              ...
              <button onClick={clearParams} className='text-base text-ankerblue md:hidden block'>Clear</button>
            </div>
            {jobCategory === JOB_CATEGORY.FULLTIME ? (
              <>
                <div className='flex flex-col gap-3 w-full'>
                  ...
                  <div className='px-[10px] py-3 border w-full border-gray-300 rounded-md flex flex-row items-center bg-white'>
                    <select id='officeType' onChange={(e) => dispatch(setOfficeType(e.target.value))} defaultValue={jobState.officeType} className='text-neutral-400 bg-white focus:outline-none border-none focus:ring-0 w-full p-0'>
                ...
                <div className='flex flex-col gap-3 w-full'>
                  ...
                  <div id='workExperience' className='px-[10px] py-3 border w-full border-gray-300 rounded-md flex flex-row items-center bg-white'>
                    <select onChange={(e) => dispatch(setExperience(e.target.value))} defaultValue={jobState.experience} className='text-neutral-400 bg-white focus:outline-none border-none focus:ring-0 w-full p-0'>
              ...
              <>
                ...
                <div className='flex flex-col gap-3 w-full'>
                  ...
                  <div className='px-[10px] py-3 border w-full border-gray-300 rounded-md flex flex-row items-center bg-white'>
                    <select id='internshipType' onChange={(e) => dispatch(setInternshipType(e.target.value))} defaultValue={jobState.internshipType} className='text-neutral-400 bg-white focus:outline-none border-none focus:ring-0 w-full p-0'>
            ...
            <button className='ml-auto flex flex-row items-center sm:py-3 sm:px-4 py-2 px-3 justify-center gap-2 w-fit bg-primaryblue border-[1px] border-primaryblue rounded-md text-white'>
              <p onClick={handleMobileApply} className='font-semibold sm:text-base text-sm'>Apply</p>
            </button>
          </div>
        </div>
      </div>

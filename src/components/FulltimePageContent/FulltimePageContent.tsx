...
import { clearAllQueryParams, getAllFulltimeJobs, getAllFulltimeJobsByFilter, setOpenFilter, setTitle } from '@/lib/features/jobs/jobSlice';
...
const FulltimePageContent = () => {
  ...
  const { fulltimeJobs, officeType, experience, minSalary, title, openFilter } = jobState;
  ...
  async function getFulltimeJobs() {
    const response = await dispatch(getAllFulltimeJobs());
  ...
  async function getFulltimeJobsByFilter(query: JobRequestByFilter) {
    const response = await dispatch(getAllFulltimeJobsByFilter(query));
    if(response.payload?.data == 401) {
      router.push('/signin');
      dispatch(clearToken());
      dispatch(logout());
  ...
  useEffect(() => {
    ...
    if(jobTitle || jobType || workExperience || minSalary) getFulltimeJobsByFilter(responseObject);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [queryParams]);
  return (
    <>
            ...
            <div className='w-screen md:w-[calc(100dvw-260px)] h-full flex flex-col xl:flex-row md:p-8 p-6 gap-8 sm:mt-0 mt-28 no-scrollbar'>
              <div className='w-full flex flex-col gap-6 sm:gap-8'>
                <div className='bg-background rounded-md p-6 w-full h-fit hidden sm:flex flex-col gap-4'>
                  ...
                  {fulltimeJobs && <p className='text-sm text-slate-500 ml-auto'>{`${fulltimeJobs.length} results Found`}</p>}
                </div>
              ...
              <JobFilter jobCategory={JOB_CATEGORY.FULLTIME} handleMoreFilters={handleMoreFilters} clearParams={clearParams} />
            </div>
          </DashboardWrapper>
        </div>
      )}
...
export default FulltimePageContent;
